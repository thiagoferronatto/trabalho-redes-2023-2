...
class GameState:
    def __init__(self):
        ...
        num_jogadores = 2
        self.player = [
            {"pokemon": [], "hp_data": [], "name": str} for _ in range(num_jogadores)
        ]
    ...
    def simulate_battle(self):
        ...
        while len(pokemons[0]) > 0 and len(pokemons[1]) > 0:
            print(
                f"Currently battling pokemon: {pokemons[0][0]['name']['english']} and {pokemons[1][0]['name']['english']}"
            )
            while hps[0] > 0 and hps[1] > 0:
                ...
                current_damage = self.calculate_damage(
                    pokemon_to_attack, pokemon_to_be_attacked
                )
                print(
                    f"{pokemon_to_attack['name']['english']} caused {current_damage} to {pokemon_to_be_attacked['name']['english']}"
                )
    ...
    def calculate_type_effectiveness(self, pokemon_a_types, pokemon_b_types):
        ...
        for ef_a in pokemon_a_types:
            for ef_b in pokemon_b_types:
                result *= type_effectiveness[ef_a].get(
                    ef_b, 1.0
                )  # caso não haja nenhuma entrada no dicionário para o tipo atual, multiplicamos por 1.0
    ...
    def calculate_damage(self, pokemon_a, pokemon_b):
        ...
        critical = (
            2 if modifier > 0.15 else 1
        )  # se o numero aleatório for menor que 0.15, o dano crítico será aplicado (15% de chance)
        effectiveness = self.calculate_type_effectiveness(
            pokemon_a["type"], pokemon_b["type"]
        )  # calculando o multiplicador de efetividade de tipos
        randomness = random.uniform(0.85, 1.0)
        upper_part = (
            (
                (((2 * 40 * critical) / 5) + 2)
                * 40
                * (pokemon_a["base"]["Attack"] / pokemon_b["base"]["Defense"])
            )
            / 50
        ) + 2
