...
def save_users(users, file_path):
    ...
    for user in users:
        file.write(f"{user.get("username")}{IAS_DB_SEPARATOR}{user.get("password_hash")}{IAS_DB_SEPARATOR}{user.get("name")}\n")
    file.close()
...
def main():
    ...
    while True:
        ...
        if matching_user:
            try:
                hasher.verify(matching_user[0].get("password_hash"), password)
